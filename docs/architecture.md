...
**Redirect convention** — Mutations (`store`, `update`, and sheet-triggered actions like `assign`, `moveOut`, `renew`) return **`back()`**, not `to_route('…index')`. The UI is sheet-driven: a form opens as a `<Sheet>` over a list _or_ a detail page and closes client-side on success, so `back()` keeps the user on whichever page they triggered it from, with fresh data, and avoids hardcoding a destination. Inertia converts the `PUT`/`PATCH`/`DELETE` redirect to 303 automatically. Exceptions that stay explicit: **`destroy`** returns `to_route('…index')` (redirecting to the just-deleted record's page would 404); **resources edited on their own page** rather than in a sheet (roles have `create`/`edit` pages, so `RoleController` returns `to_route('roles.index')` — `back()` would leave you on the form); and auth/cross-context flows (e.g. `completeInvitation` → `login`) target their real destination. Rule of thumb: sheet-driven mutation → `back()`; dedicated-page or navigate-away mutation → `to_route(...)`. When testing a `back()` action, set `->from(route(...))` on the request so the assertion has a referer to land on.
